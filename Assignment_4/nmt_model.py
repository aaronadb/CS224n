...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens,_=nn.utils.rnn.pad_packed_sequence(sequence=enc_hiddens,batch_first=False)
        enc_hiddens=enc_hiddens.permute(1,0,2)
        concat_h=torch.cat((last_hidden[0],last_hidden[1]),dim=1)
        init_decoder_hidden=self.h_projection(concat_h)
        concat_c=torch.cat((last_cell[0],last_cell[1]),1)
        init_decoder_cell=self.c_projection(concat_c)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(tensor=Y,split_size_or_sections=1,dim=0):
            ...
            o_prev=o_t
        #combined_output=torch.zeros(len(combined_outputs),batch_size,combined_outputs[0].size()[1])
        #combined_output=torch.stack(tuple(i for i in combined_outputs))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_temp=torch.unsqueeze(dec_hidden,dim=2)
        e_t=torch.bmm(enc_hiddens_proj,dec_hidden_temp)
        ...
        if enc_masks is not None:
            e_t.data.masked_fill_(enc_masks.byte().to(torch.bool), -float('inf'))
        ...
        alpha_t_temp=torch.unsqueeze(alpha_t,dim=1)
        a_t=torch.bmm(alpha_t_temp,enc_hiddens)
