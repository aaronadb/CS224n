...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
            sent_rep=[]
            for word in sent:
                word_rep=[self.char2id['{']]
                for i in word:
                    word_rep+=[self.char2id[i]]
                word_rep+=[self.char2id['}']]
                sent_rep+=[word_rep]
            word_ids+=[sent_rep]
        return word_ids
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        temp=pad_sents_char(temp,self.char2id['<pad>'])
        sents_var=torch.tensor(temp,device=device)
        sents_var=torch.reshape(sents_var,(sents_var.size()[1],sents_var.size()[0],sents_var.size()[2]))
        return sents_var
