...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x=torch.empty((w.size()[0],w.size()[1]*self.embeddings.size()[1]))
        for i in range(0,w.size()[0],1):
            y=torch.index_select(self.embeddings,0,w[i,:])
            y=y.view(1,-1)
            x[i]=y
    ...
    def forward(self, w):
        ...
        h=torch.matmul(w,self.embed_to_hidden_weight)+self.embed_to_hidden_bias
        rel=nn.ReLU()
        h=rel(h)
        d=self.dropout(h)
