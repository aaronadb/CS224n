...
def minibatch_parse(sentences, model, batch_size):
    ...
    while(len(unfinished_parses)!=0):
        ...
        for j in range(0,len(minibatch),1):
            minibatch[j].parse_step(next_transitions[j])
        for j in range(0,len(minibatch),1):
            if(len(minibatch[j].buffer)==0 and len(minibatch[j].stack)==1):
                unfinished_parses.remove(minibatch[j])
        
    return [i.dependencies for i in partial_parses]
